...
const JokerRap = () => {
  ...
  const fetchCandidates = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/joker-rap`);
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // Enregistrer le vote
      await axios.post(`${API_BASE_URL}/joker-rap/vote`, {
  ...
  const openImagePopup = (candidate) => {
    setSelectedImage({
      url: `${API_BASE_URL}/joker-rap/photo/${candidate.id}`,
      name: candidate.nomJokerRap
    });
      ...
      <div className="candidates-list">
        {candidates.map((candidate) => (
          <div key={candidate.id} className="candidate-card">
            ...
            >
              <img
                src={`${API_BASE_URL}/joker-rap/photo/${candidate.id}`}
                alt={candidate.nomJokerRap}
                className="rapper-image"
                onError={(e) => {
                  e.target.src = "https://via.placeholder.com/300x300/1a1a1a/ffffff?text=Image+Non+Disponible";
                }}
              />
              <div className="rapper-overlay">Voir l'image</div>
            </div>

            <div className="candidate-info">
              <h3>{candidate.nomJokerRap}</h3>
        ...
        <div className="payment-modal-overlay">
          <div className="payment-modal-content compact">
            ...
            <div className="modal-header">
              <h3>Voter pour {selectedCandidate.nomJokerRap}</h3>
              <button
      ...
      {showImagePopup && selectedImage && (
        <div className="image-popup-overlay" onClick={() => setShowImagePopup(false)}>
          <div className="image-popup-content" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="popup-subtitle">Joker Rap</p>
          </div>
        </div>
      )}
    </div>
